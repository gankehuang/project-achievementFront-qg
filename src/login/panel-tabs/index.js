...
$(function () {
    ...
    $('.btn-next').on('click', function (e) {
        ...
        if (!$(hrefName).find("form").valid()) {
            ...
            save($(this).data('tabindex'));
            //调用判断成果分类下拉框是否选择完整 才走下一步
        }
    });
    //全部的返回按钮
    ...
    //一级分类
    getBasicInfo(Util.service.GetAllTopClassifyNoIRM, "#ClassifyId1", "一级分类");
    // //获取所有新产业类型
    ...
    //if (!id && loginData && loginData.CorporateName) {
       if (!id && loginData) {
           ...
           var useid = loginData.ID;
        $.ajax({
            url: Util.service.GetUserByUserID,
            type: 'POST',
            data: { "userID": useid },
            beforeSend: function () {
                Util.pageLoading();
            ...
            success: function (data) {
                ...
                console.info(data.resultData[0].Name);
                Util.pageinit();
                if (data.success === 1) {
                    $("#IPRForm").find("[name=OwnerOrganization]").val(data.resultData[0].Name)
    ...
    //$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success === 1) {
                    data.resultData = JSON.parse(data.resultData);
                    //var irm =data.resultData;
                    //irm.push({ id: 0, name: "无" });
    ...
    function getAchDetail() {
        ...
        for (var key in isInitOver) {
            isInitOver_str += isInitOver[key].toString()+'--';
        }
        console.log(isInitOver_str)
        ...
        if (!over && !ClassifyIdList && flag_init == 0) {
            flag_init++;
            console.info("init")
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success === 1) {
                        data.resultData = JSON.parse(data.resultData);
                        console.log(JSON.parse(data.resultData.Achievement));
                        console.log(JSON.parse(data.resultData.ImageVideo));
                        ...
                        $("#ClassifyId1").trigger("change");//触发改变事件
                       ...
                       // for (var i in arr_number) {
                            ...
                            if ($ele.parent().next().val().trim()) {
                                ...
                                $('[name=IPRNumber]').eq(AchievementImageIPRNumber[i].IPRNumberTypeID - 1).after(ele_IPR);
                                $('[name=IPRNumber]').eq(AchievementImageIPRNumber[i].IPRNumberTypeID - 1).prop("required", true);
                            }
                        }
                        ...
                        //console.log('33333333333', ImageVideo);
                        ...
                        for (var i in ImageVideo) {
                            if (ImageVideo[i].type == '0') {
                                ...
                                if (ImageVideo[i].MainPicId == 1) {
                                    mainimgList.push({ Url: Util.urlpath + ImageVideo[i].url, ID: ImageVideo[i].id });
                                }
                                else {
                                    imgList.push({ Url: Util.urlpath + ImageVideo[i].url, ID: ImageVideo[i].id });
                                }
                        ...
                        //判断是否是其他品种 加载其他具体名称
                        var last = ClassifyIdList.length - 1;//获取最后一个种类下拉框的下标
                        var lastclass = ClassifyIdList[last];//最后一个种类
                        var lastname = lastclass.ClassifyName//文本值
                        //console.log(lastname)
                        var str = "其他";
                        var othername = Achievement.OtherName;//其他种类具体名称
                        setTimeout(setOthername, 1000);//1分钟后待种类下拉框加载完毕 反填具体名称
                        function setOthername() {
                            if (lastname.indexOf(str) >= 0) {
                                //console.log($("#qita input[name='otherName']"))
                                $("#qita input[name='otherName']").css("background-color", "")
                                $("#qita input[name='otherName']").val(othername)
                            }
                            else {
                                $("#qita").hide();
                            }
    ...
    $("#IPRNumberSelect").on("change", "[name=IPRNumberTypeID]", function () {
        ...
        for (i = 0; i < objarray; i++) {
            if (str[i].checked == true) {
                chestr += str[i].value + ",";
                //if ($(str[i]).parent().siblings("[name=IPRNumber]").val() == null || $(str[i]).parent().siblings("[name=IPRNumber]").val() == "") {
                //    Util.warning("show", "请填写对应知识产权编号内容");
                //    return;
                //} else {
                //    textzs += $(str[i]).parent().siblings("[name=IPRNumber]").val() + ",";
                //}
            }
        }
        //!=-1说明用户选择了无
        if (chestr.indexOf("6") != -1) {
            if (chestr.length > 2) {
                Util.warning("show", "选择无之后不可再选择其他选项", function () {
                    $(str).prop("checked", false);
                });
                return;
            }
            else {
                //if (textzs == null || textzs == "" || textzs == ",") {
                //    Util.warning("show", "请填写对应知识产权编号内容");
                //    return;
                //}
            }
            ...
            if (chestr.length <1) {
                Util.warning("show", "请至少选择一项知识产权编号进行填写", function () {
                    $(str).prop("checked", false);
                });
                return;
            }
    ...
    $(".btn-save").on('click', function (e) {
        ...
        $("#IPRNumberSelect .form-inline").each(function () {
            ...
            //判断如果用户选择的是无的时候不允许填写其他的，当用户选择填写的时候最少要填写一个
            //选择无，其他选项不允许填写
        ...
        if (AchievementImageIPRNumber == null || AchievementImageIPRNumber == "") {
            message += "</br>若有则请至少填写一项知识产权编号，若无则选择无";
        }
        if (classdrp() == false || filelength() == false || form1.AchieveName == "" || form1.AchieveName == null || form1.MaturityID == null || form1.MaturityID == "" || form1.MaturityDesc == null || form1.MaturityDesc == "" || form2.IPRDesc == null || form2.IPRDesc == "" || form2.Award == null || form2.Award == "" || form3.RegistNumber == null || form3.RegistNumber == "" || form3.ExpectedTransPrice == null || form3.ExpectedTransPrice == "" || AchievementImageIPRNumber == null || AchievementImageIPRNumber=="") {
        ...
        /*$("#imgBoxs2").find(".img-responsive").each(function (i) {
            ImageVideo.push({ "ImageVideoID": $(this).data("id"), "Index": ImageVideo.length, "MainPicId": $(this).data("id") })
        })
        len4 = ImageVideo.length - len1 - len2 - len3;*/
        if (len2 < 1) {
        ...
        if(dataJson.Achievement.IsDiscussPrice == "0"){
            dataJson.Achievement.IsCompetePrice = "1";
        }else{
            dataJson.Achievement.IsCompetePrice = "0";
        }
            ...
            success: function (data) {
                ...
                if (data.success === 1) {
                    ...
                    Util.success("show", "保存成功", function () {
                        location.href = "/login/notification/index.html";
                    });
                    //location.href = "/login/notification/index.html";
                } else {
    ...
    $('#imgUpload1').fileupload({
        ...
        add: function (e, data) {
            ...
            var b = a.substr(idx+1).toUpperCase();
            if (b != 'JPG' && b != 'PNG' && b != 'JPEG' && b != 'GIF'){
                Util.warning("show", "文件类型只能为图片");
                return;  
            } 
            ...
            if (data.success) {
                ...
                for(var i=0; i<res.length; i++) {
                    console.log(res[i]);
                    res[i].Url = Util.urlpath + res[i].Url;
                }
                data.resultData = res;
...
function save(tabindex) {
    ...
    if (tabindex >=1) {
        //调用判断成果分类下拉框 是否选择完整
        if (classdrp() == false) {
            Util.warning("show", "请完善成果类别！");
            return;
        }
        //调用判断是否上传成果简介
        if (filelength() == false) {
            Util.warning("show", "请上传成果简介！");
            return;
        }
    }
    if (tabindex >= 2) {
        //if (knowleadg() == false) {
        //    Util.warning("show", ms);
        //    return;
        //}
        //knowleadgfirst();
        ...
        for (i = 0; i < objarray; i++) {
            ...
            if (str[i].checked == true) {
                chestr += str[i].value + ",";
                $(str[i]).parent().siblings("[name=IPRNumber]").prop("required", true)
            }
        }
        if (chestr.indexOf("6") != -1) {
            if (chestr.length > 2) {
    ...
    $("#IPRNumberSelect .form-inline").each(function () {
        ...
        console.log(form4);
        //form4.ImageVideo = form4.ImageVideo.concat(form2.ImageVideo);
        var dataJson = $.extend({}, {
            "Achievement": $.extend({}, form1, form2, form3, form4, { "UserID": Util.getUserId() })
        }, { "Keywords": Keywords }, {
            "AchievementClassify": AchievementClassify,//成果分类
        ...
        /*if(form2.ImageVideo) {
            dataJson.ImageVideo.push(form2.ImageVideo[0]);
        }*/
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success === 1) {
                    ...
                    //console.log(classdrp() && filelength());
                    if (tabindex == 1) {
                        if (classdrp() && filelength()) {
                            $('#myTab1 li.active a').parent().next().find("a").tab('show');//下一步

                        }
                    }
                  else  if (tabindex == 2) {
                      if (classdrp() && filelength() && knowleadg()) {
                            $('#myTab1 li.active a').parent().next().find("a").tab('show');//下一步
                    ...
                    for(var i=0; i<resultData.length; i++) {  
                        a += '<a href="/login/panel-tabs/index.html?id='+ resultData[i].ID +'">'+ resultData[i].Name +'</a><br/>'
                    }
                    $('.sameCategory').html(a);
                    $('#sameCategory').modal('show');
                } else {
                    Util.error("show", data.message)
                }
            },
...
function classdrp() {
    ...
    var a = $("#ClassifyId1").val()
    var b = $("#ClassifyId2").val()
    var c = $("#ClassifyId3").val()
    var d = $("#ClassifyId4").val()

    //下拉框显示文本
    var a1 = $("#ClassifyId1 :checked").text();
    var b1 = $("#ClassifyId2 :checked").text();
    var c1 = $("#ClassifyId3 :checked").text();
    var d1 = $("#ClassifyId4 :checked").text();

    var str = "其他";

    if ((a == 1 && b == 9) || (a == 2 && b == 151)) {
        if (d == null || d == "") {
            $("#s1").html("请继续完善成果分类信息");
            flag = false;
        }
        ...
        if (d1.indexOf(str) >= 0) {
            var size = $("#qita").find("input").size()
            if (size == 0) {
                $("#qita").append('<input type="text" name="otherName" placeholder="请填写其他类别的具体名称，具体名称不能为空！" style="width:500px;height:30px;border-radius:5px;" required>');
                $("#qita").show();
            }
            $("#qita").show();
...
function knowleadg() {
    ...
    var str = document.getElementsByName("IPRNumberTypeID");
    var str1 = document.getElementsByName("IPRNumberTypeID1");
    var textzs = "";
    var objarray = str.length;
    var chestr = "";
    for (i = 0; i < objarray; i++) {
...
//验证知识产权编号是否填写
function knowleadgfirst() {
    $(this).find("[name=IPRNumber]").each(function () {
            $(this).siblings("[name=IPRNumber]").prop("required", $(this).is(":checked"));
    })
}
