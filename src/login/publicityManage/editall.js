...
$(function () {
    ...
    if(id){
        ...
        $('#breadcrumb').append('<li><a href="javascript:void(0);"><span class="icon icon-beaker"> </span><em>编辑公告</em></a></li>');
    }
    else{
        $('#breadcrumb').append('<li><a href="javascript:void(0);"><span class="icon icon-beaker"> </span><em>公告发布</em></a></li>');
    ...
    //获取已通过公示
    getBasicInfo(Util.service.GetAllPublicityPassedByUserID, "#SelectAchID", "请选择", {
        userId: loginData.ID,
        VisitID: 1
    }, null, function(){
        $('#SelectAchID').selectpicker({
    ...
    $('#imgUpload').fileupload({
        ...
        add: function (e, data) {
            ...
            if (data.success) {
                ...
                $('#UploadImage .fileinput-button').hide();
            } else {
    ...
    if( $('#UploadPath').html() != '' ) {
        $('#contractFJUpload').attr('required', false);
    }
    ...
    $("#clearImgs").on("click", function (e) {
        ...
        e.preventDefault();
        $("#imgBoxs").html("");
        $('#UploadImage .fileinput-button').show();
    });
...
//合同时间是否永久单选
$("[name=PubPermanent]").on("change", function () {
    if ($(this).val() == 1)
    {
        $("#timeend1").hide();
        $("#timeend2").hide();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success === 1) {
                    ...
                    $("#form_all").formFieldValues(data.resultData);
                    // 图片
                    ...
                    var selectData = data.resultData.AchID.split(',')
                    $('#SelectAchID').selectpicker('val', selectData);  //关联到已发布公示回填
                    ...
                    //合同时间
                    console.info(data.resultData.PubPermanent);
                    if (data.resultData.PubPermanent == 1) {
                        $("#timeend1").hide();
                        $("#timeend2").hide();
                    }
                    else {
...
function saveAll(){
    ...
    var form = $('#form_all').serializeJson(); 
    console.log('&&&&&&&&&&&', form);
    ...
    var file_path = $('#UploadPath a').attr('href');
    
    form.UploadPath = file_path.substring(file_path.indexOf('/FJ/'));
    console.log(form)
    var dataJson = $.extend({}, {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { jsonStr: JSON.stringify(dataJson),"PublicityID":id, 'Type':1, 'VisitID': manageVisitID },
        beforeSend: function () {
            Util.pageLoading();
        },
        success: function (data) {
